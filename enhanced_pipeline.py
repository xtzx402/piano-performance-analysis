...
ax.set_title(f'{k_fold}-Fold Cross-Validation Results')
ax.legend()
...
ax.set_title('ANOVA: Which MFCC coefficients differ significantly?')
ax.legend()
ax.grid(True, alpha=0.3, axis='y')
plt.tight_layout()
...
if X_test_list:
    ...
    ax.set_ylabel('Accuracy')
    ax.set_title('Model Generalization: Can it recognize NEW performers?')
...
ax.set_title('Feature Heatmap: Which features distinguish performers?')
plt.tight_layout()
